-- File: sql/staging/create_stg_shopify_order_items_hybrid_dry_run.sql
-- Model: raw_load.stg_shopify_order_items_hybrid_dry_run
-- Purpose:
-- Simulate staging.stg_shopify_order_items using the Shopify hybrid raw order
-- candidate instead of production raw.shopify_orders.
--
-- Important notes:
-- - This does not modify raw.shopify_orders.
-- - This does not modify staging.stg_shopify_order_items.
-- - This dry-run table is isolated in raw_load.

CREATE OR REPLACE TABLE `mischief-made-analytics.raw_load.stg_shopify_order_items_hybrid_dry_run` AS
